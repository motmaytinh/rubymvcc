...
class MyTest < Test::Unit::TestCase
  ...
  def test_read_uncommitted
    ...
    c2.exec_command('begin', nil)

    c1.exec_command('set', 'x', 'hey')
  ...
  def test_read_committed
    ...
    c3.exec_command('begin', nil)

    # Local change is visible locally.
    c3.exec_command('set', 'x', 'yall')

    res = c3.exec_command('get', 'x')
  ...
  def test_repeatable_read
    ...
    res = c4.exec_command('get', 'x')
    assert_equal('hey', res, 'c3 cannot get x')

    c4.exec_command('delete', 'x')
    c4.exec_command('commit', nil)

    # But the delete is visible to new transactions now that this has been committed.
    c5 = database.new_connection
    c5.exec_command('begin', nil)

    assert_raise RuntimeError do
      c5.exec_command('get', 'x')
    end
  end
  ...
  def test_snapshot_isolation
    ...
    c2.exec_command('set', 'x', 'hey')

    assert_raise RuntimeError do
      c2.exec_command('commit', nil)
    end

    c3.exec_command('set', 'y', 'hey')
    c3.exec_command('commit', nil)
  end
